...
net inputs (w:weights) f =
    let currentLayer = map f (layer inputs w)
        otherLayers  = net currentLayer weights f
    in  currentLayer : otherLayers
...
layer _ [] = []
layer inputs (w:weights) =
    let currentNeuron = neuron inputs w
        nextNeurons   = layer inputs weights
    in  currentNeuron : nextNeurons

-- Returns the value of a neuron created with the specified inputs and weights
neuron :: [Double] -> [Double] -> Double
neuron inputs weights = sum (zipWith (*) inputs weights)
